...
%% Terminal condition --> intensity too high
if NextObs(1) == 2
    IsDone = true;
else
    IsDone = false;
...
%if the values doesn't change for 10 consecutive values stop the simulation
n = 5;
if count > n %consider at least 10 iterations
    check_A = variables(9,end-n:end); %consider last 10 values
    check_PW = variables(10,end-n:end); %consider last 10 values
...
%perform plot
% figure(1),
% subplot(10,1,1),plot(1:count, variables(1,:),'--o'),title('Amplitude');...
% subplot(10,1,2),plot(1:count, variables(2,:),'--o'),title('Pulse Width');...
% subplot(10,1,3),plot(1:count, variables(3,:),'--o'),title('Intensity');...
% subplot(10,1,4),plot(1:count, variables(4,:),'--o'),title('Type');...
% subplot(10,1,5),plot(1:count, variables(5,:),'--o'),title('Location');...
% subplot(10,1,6),plot(1:count, variables(6,:),'--o'),title('ActionAmplitude');...
% subplot(10,1,7),plot(1:count, variables(7,:),'--o'),title('ActionPulseWidth');...
% subplot(10,1,8),plot(1:count, variables(8,:),'--o'),title('Reward');...
% subplot(10,1,9),plot(1:count, variables(9,:),'--o'),title('A_index');...
% subplot(10,1,10),plot(1:count, variables(10,:),'--o'),title('PW_index');

if count == 200 || IsDone == true
    array = [ intensity , type , location , m , q , amplitude , pulsewidth , count - n];
    array
%     load resultsPlus1_new.mat
%     temp = resultsPlus1_new.matrix;
%     current = size(temp,1);
%     temp(current+1,:) = array;
%     resultsPlus1_new.matrix = temp;
%     save('resultsPlus1_new','resultsPlus1_new');
end

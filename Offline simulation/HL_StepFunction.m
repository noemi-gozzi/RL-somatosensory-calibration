function [NextObs,Reward,IsDone,LoggedSignals] = HL_StepFunction(Action,LoggedSignals)
...
%positive
if NextObs(1) == 1 && NextObs(2) == 0 && NextObs(3) == 0
    Reward = 15;
...
elseif NextObs(1) == 0 && NextObs(2) == 0 && NextObs(3) == 0
    Reward = 4;
...
%% perform plot
figure(2),
subplot(3,2,1),plot(1:count, variables(1,:),'r--o'),title('Amplitude'),xlim([1,inf]);...
subplot(3,2,2),plot(1:count, variables(2,:),'r--o'),title('Pulse Width'),xlim([1,inf]);...
subplot(3,2,3),plot(1:count, variables(3,:),'r--o'),title('Intensity'),xlim([1,inf]),yticks([-1,0,1,2]),yticklabels(["NotPerceived","Low","High","Painful"]);...
subplot(3,2,4),plot(1:count, variables(4,:),'r--o'),title('Type'),xlim([1,inf]),yticks([-1,0,1]),yticklabels(["NotPerceived","Pleasant","Unpleasant"]);...
subplot(3,2,5),plot(1:count, variables(5,:),'r--o'),title('Location'),xlim([1,inf]),yticks([-1,0,1]),yticklabels(["NotPerceived","Somatotopic","NotSomatotopic"]);...
subplot(3,2,6),plot(1:count, variables(8,:),'r--o'),title('Reward'),xlim([1,inf]);...
sgtitle('High level simulation') 


if count == 200 || IsDone == true
    high_results = [amplitude , pulsewidth];
    save('HighResults','high_results');
end
